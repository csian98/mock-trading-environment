...
class Trader:
    ...
    def trade(self):
        ...
        for symbol in self.symbols:
            ...
            scores[symbol] = \
                (max(pp) - self.prices[symbol]) / self.prices[symbol]
        
        for symbol, count in self.counts.items():
            if count != 0 and scores[symbol] < -0.01:
        ...
        for symbol, _ in candidate:
            balance += self.counts[symbol] * self.prices[symbol]

        for symbol, score in candidate:
    ...
    def report(self, day: int) -> None:
        ...
        for symbol, count in self.counts.items():
            if count != 0:
                print(f"{symbol}: {self.prices[symbol]:.2f} X {count} = {self.prices[symbol] * count:.2f}")
